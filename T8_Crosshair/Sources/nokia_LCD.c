...
//   spi_txCommand(P_BSTRON);                   // Boost On
   ...
   lcd_spiTxData(0x02);             // P3: 0x02 = Grayscale -> 16 (selects 12-bit colour, type A)
...
static void lcd_setXY(int x, int y) {
  ...
  * @author James P Lynch July 7, 2007
  */
void lcd_drawPixel(int x, int y, int colour) {
#ifdef PHILIPS
   lcd_setXY(x, y);
   lcd_spiTxCommand(P_RAMWR);
   // Construct RRRRGGGGBBBB=>RRRRGGGG|BBBBxxxx in 3 bytes = 2 pixels
   lcd_spiTxData((unsigned char)(colour >> 4));
   lcd_spiTxData((unsigned char)(((colour & 0x0F) << 4) | 0x00));
   ...
   // Construct RRRRGGGGBBBB=>RRRRGGGG|BBBBxxxx in 3 bytes = 2 pixels
   lcd_spiTxData((unsigned char)(colour >> 4));
   lcd_spiTxData((unsigned char)(((colour & 0x0F) << 4) | 0x0F));
  ...
  */
void lcd_drawLine(int x0, int y0, int x1, int y1, int colour) {
   int dy = y1 - y0;
   int dx = x1 - x0;
   int stepx, stepy;
   if (dy < 0) {
  ...
  */
void lcd_drawRect(int x0, int y0, int x1, int y1, int fill, int colour) {
   ...
   if (fill) {
      ...
      for (i = 0; i < ((((xmax - xmin + 1) * (ymax - ymin + 1)) / 2) + 1);
            i++) {
         // use the colour value to output three data bytes covering two pixels
         lcd_spiTxData((colour >> 4) & 0xFF);
         lcd_spiTxData(((colour & 0xF) << 4) | ((colour >> 8) & 0xF));
         lcd_spiTxData(colour & 0xFF);
      }
   } else {
      // best way to draw unfilled rectangle is to draw four lines
      lcd_drawLine(x0, y0, x1, y0, colour);
      lcd_drawLine(x0, y1, x1, y1, colour);
      lcd_drawLine(x0, y0, x0, y1, colour);
      lcd_drawLine(x1, y0, x1, y1, colour);
   }
}
  ...
  *   @author James P Lynch July 7, 2007
  */
void lcd_putChar(char c, int x, int y, FontSize fontSize, int fColour, int bColour) {
   unsigned int nCols;
   unsigned int nRows;
   unsigned int nBytes;
 ...
 /*!  Draws a null-terminates character string at the specified (x,y) address and colour
  *
  *  @param pString   = pointer to character string to be displayed
  *  @param x         = row address (0 .. 131)
  *  @param y         = column address (0 .. 131)
  *  @param fontSize  = font pitch (FontSmall, FontMedium, FontLarge)
  ...
  */
void lcd_putStr(const char *pString, int x, int y, FontSize fontSize, int fColour, int bColour) {
   // loop until null-terminator is seen
   while (*pString != 0x00) {
      // draw the character
      lcd_putChar(*pString++, x, y, fontSize, fColour, bColour);
...
void lcd_setContrast(uint8_t setting) {
  ...
  * @param colour       = 12-bit colour value rrrrggggbbbb
  * @param circleType   = Indicates which segments of the circle to draw
  *
  ...
  */
void lcd_drawCircle(int x0, int y0, int radius, int colour, int circleType) {
   int f = 1 - radius;
   int ddF_x = 0;
   int ddF_y = -2 * radius;
   int x = 0;
   ...
   if (circleType&(SECTOR_315_360|SECTOR_0_45)) {
      lcd_drawPixel(x0 + radius, y0, colour);  // 0,360
   }
   if (circleType&(SECTOR_45_90|SECTOR_90_135)) {
      lcd_drawPixel(x0, y0 + radius, colour);  // 90
   }
   if (circleType&(SECTOR_135_180|SECTOR_180_225)) {
      lcd_drawPixel(x0 - radius, y0, colour);  // 180
   }
   if (circleType&(SECTOR_225_270|SECTOR_270_315)) {
      lcd_drawPixel(x0, y0 - radius, colour);  // 270
   }
   while (x < y) {
      if (f >= 0) {
         y--;
      ...
      if (circleType&SECTOR_0_45) {
         lcd_drawPixel(x0 + y, y0 + x, colour);   // 0-45
      }
      if (circleType&SECTOR_45_90) {
         lcd_drawPixel(x0 + x, y0 + y, colour);   // 45-90
      }
      if (circleType&SECTOR_90_135) {
         lcd_drawPixel(x0 - x, y0 + y, colour);   // 90-135
      }
      if (circleType&SECTOR_135_180) {
         lcd_drawPixel(x0 - y, y0 + x, colour);   // 135-180
      }
      if (circleType&SECTOR_180_225) {
         lcd_drawPixel(x0 - y, y0 - x, colour);   // 180-225
      }
      if (circleType&SECTOR_225_270) {
         lcd_drawPixel(x0 - x, y0 - y, colour);   // 225-270
      }
      if (circleType&SECTOR_270_315) {
         lcd_drawPixel(x0 + x, y0 - y, colour);   // 270-315
      }
      if (circleType&SECTOR_315_360) {
         lcd_drawPixel(x0 + y, y0 - x, colour);   // 315-360
      }
   }
}
